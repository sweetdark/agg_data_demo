...
namespace agg
{
    ...
    public:
        ...
        //--------------------------------------------------------------------
        template<class Scanline> bool sweep_scanline(Scanline& sl)   //逐行光栅化
        {
            for(;;)
            {
                if(m_scan_y > m_outline.max_y()) return false;  //判断行数是否超出界限
                sl.reset_spans();   
                unsigned num_cells = m_outline.scanline_num_cells(m_scan_y);  //取出单元格个数
                ...
                while(num_cells)
                {
                    ...
                    if(num_cells && cur_cell->x > x)
                    {
                        ...
                        if(alpha)
                        {
                            sl.add_span(x, cur_cell->x - x, alpha);   //光栅化到scanline_u8容器里的每一行中。x是行坐标，cur_cell->x - x 是长度， alpha是cover值
                        }
                    }
                }
